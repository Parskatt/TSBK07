...
void main(void)
{
	...
	for (int i = 0; i < 4;  i++)
	{
		vec3 reflectedLightDirection = reflect(-lightSourcesDirPosArr[i], normalize(outNormal));
		...
		diffuse_component = dot(normalize(outNormal), lightSourcesDirPosArr[i]);
		diffuse_component = clamp(diffuse_component, 0, 1);
		temp = temp + (diffuse_component*0.5 + specularStrength)*lightSourcesColorArr[i];
	}
