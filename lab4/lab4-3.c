...
// Normal vectors. You need to calculate these.
      vec3 n = CrossProduct((vertexArray[((x + z * tex->width)*3 + 2] - vertexArray[(x + z * tex->width)*3 + 1]), (vertexArray[(x + z * tex->width)*3 + 1] - vertexArray[(x + z * tex->width)*3 + 0]));
      n = n/Normalize(n);
			normalArray[(x + z * tex->width)*3 + 0] = n.x; //0.0;
			normalArray[(x + z * tex->width)*3 + 1] = n.y; //1.0;
